...
public class Player : MonoBehaviour
{
    ...
    public bool isWithinPole;
    public Pole LastPole;
    public Pole NextPole => LastPole == PlanetManager.planet.northPole
        ? PlanetManager.planet.southPole : PlanetManager.planet.northPole;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.A)) {
            if (canTurn && LastPole) {
                ...
                (location.track, nextDir, location.lane, location.theta) =
                    LastPole.InterpretTurn(location.track, currentDir, location.lane, Turn.Left);
        ...
        else if (Input.GetKeyDown(KeyCode.D))
        {
            if (canTurn && LastPole)
            {
                ...
                var currentDir = speed < 0 ? Heading.Forward : Heading.Backward;
                (location.track, nextDir, location.lane, location.theta) =
                    LastPole.InterpretTurn(location.track, currentDir, location.lane, Turn.Right);
                speed = (int)nextDir * Mathf.Abs(speed);
